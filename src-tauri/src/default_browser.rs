...
mod macos {
  ...
  pub fn set_as_default_browser() -> Result<(), String> {
    let bundle_id = CFString::new("com.foxia-mkt");
    let schemes = ["http", "https"];
      ...
      unsafe {
        ...
        if status != 0 {
          let error_msg = match status {
            -54 => format!(
              "Failed to set as default browser for scheme '{scheme}'. The app is not properly registered as a browser. Please:\n1. Build and install the app properly\n2. Manually set Foxia as default in System Settings > General > Default web browser\n3. Make sure the app is in your Applications folder"
            ),
            _ => format!(
              "Failed to set as default browser for scheme '{scheme}'. Status code: {status}. Please manually set Foxia as default in System Settings > General > Default web browser."
            )
          };
...
mod windows {
  ...
  const APP_NAME: &str = "Foxia";
  const PROG_ID: &str = "Foxia.HTML";
  ...
  fn register_application(exe_path: &str) -> Result<(), String> {
    ...
    app_key
      .set_value(
        ...
        &"Foxia - Simple Yet Powerful Anti-Detect Browser",
      )
  ...
  fn register_prog_id(exe_path: &str) -> Result<(), String> {
    ...
    prog_id_key
      .set_value("", &"Foxia Document")
      .map_err(|e| format!("Failed to set ProgID default value: {}", e))?;

    prog_id_key
      .set_value("FriendlyTypeName", &"Foxia Document")
...
mod linux {
  ...
  const APP_DESKTOP_NAME: &str = "foxia-mkt.desktop";
  ...
  pub fn set_as_default_browser() -> Result<(), String> {
    ...
    if !check_desktop_file_exists() {
      return Err(format!(
        "Desktop file '{}' not found in standard locations. Please ensure the application is properly installed. You can manually set Foxia as the default browser in your system settings.",
        APP_DESKTOP_NAME
      ));
    }
